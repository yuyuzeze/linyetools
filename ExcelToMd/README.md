...
- **有合并单元格** 时：按 Excel 习惯，合并区域内 **每个格子都写成左上角单元格的值**（视觉上会「重复」几格），这样才能在标准 Markdown 里表示，而 **没有** 真正的 `rowspan`/`colspan` 语义。

**空行与空表行：**

- 文档里 **连续多个空行会压成至多一行空行**。
- 表格中 **整行都没有可见文字**（按合并展开后的有效值判断）的行会 **自动省略**。
